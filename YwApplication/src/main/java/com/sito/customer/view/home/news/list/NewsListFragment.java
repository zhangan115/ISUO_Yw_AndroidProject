...
import com.sito.customer.view.home.news.safe.NewsSafeActivity;
import com.sito.customer.view.home.news.enterprise.EnterpriseActivity;
import com.sito.customer.view.home.work.increment.WorkIncrementActivity;
import com.sito.customer.view.home.work.increment.detail.IncrementDetailActivity;
...
import com.sito.customer.widget.MessageItemLayout;
...
public class NewsListFragment extends LazyLoadFragmentV4<NewsListContract.Presenter> implements NewsListContract.View
    ...
    public void onCreate(@Nullable Bundle savedInstanceState) {
        ...
        new NewsListPresenter(CustomerApp.getInstance().getRepositoryComponent().getRepository(), this);
    ...
    public void borrowSureSuccess() {
        CustomerApp.getInstance().showToast("操作成功");
    }

    private void refreshNoReadCount() {
        if (type == 0) {
            SPHelper.write(getActivity(), ConstantStr.USER_INFO,
                    CustomerApp.getInstance().getCurrentUser().getUserId() + ConstantStr.NEWS_WORK_STATE, 0);
        } else if (type == 1) {
            SPHelper.write(getActivity(), ConstantStr.USER_INFO,
                    CustomerApp.getInstance().getCurrentUser().getUserId() + ConstantStr.NEWS_ALARM_STATE, 0);
        } else if (type == 2) {
            SPHelper.write(getActivity(), ConstantStr.USER_INFO,
                    CustomerApp.getInstance().getCurrentUser().getUserId() + ConstantStr.NEWS_NOTIFY_STATE, 0);
        } else {
            SPHelper.write(getActivity(), ConstantStr.USER_INFO,
                    CustomerApp.getInstance().getCurrentUser().getUserId() + ConstantStr.NEWS_ME_STATE, 0);
        }
