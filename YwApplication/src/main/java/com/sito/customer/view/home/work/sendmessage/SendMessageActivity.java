...
import com.sito.customer.widget.FileItemLayout;
...
public class SendMessageActivity extends BaseActivity implements SendMessageContract.View {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        new SendMessagePresenter(CustomerApp.getInstance().getRepositoryComponent().getRepository(), this);
        initView();
        initData();
    }
    ...
    private void initView() {
        ...
        tvMessageType = (TextView) findViewById(R.id.tvMessageType);
        ...
        switch (v.getId()) {
            ...
            case R.id.btn_send:
                ...
                if (TextUtils.isEmpty(titleEt.getText().toString())) {
                    CustomerApp.getInstance().showToast("请输入标题");
                    return;
                }
                ...
                if (messageType == -1) {
                    CustomerApp.getInstance().showToast("请在后台配置消息类型");
                    return;
                }
        ...
        if (requestCode == REQUEST_CODE_USER && resultCode == RESULT_OK) {
            ...
            if (new File(filePath).isFile()) {
                if (new File(filePath).length() > 20 * 1024 * 1024) {
                    CustomerApp.getInstance().showToast("请选择20M以下的文件");
                    return;
                }
                fileList.clear();
