package com.sito.customer.view.home.work.inspection.work;
...
import com.sito.customer.mode.inspection.InspectionRepository;
import com.sito.customer.view.MvpFragmentV4;
import com.sito.customer.view.alarm.adduser.EmployeeActivity;
import com.sito.customer.view.home.work.inspection.report.ReportActivity;
import com.sito.customer.widget.RoomListLayout;
    ...
    public void onCreate(@Nullable Bundle savedInstanceState) {
        ...
        if (CustomerApp.getInstance().getCurrentUser().getCustomer().getCustomerConfig() != null
            ...
            for (int i = 0; i < CustomerApp.getInstance().getCurrentUser().getCustomer().getCustomerConfig().size(); i++) {
                if (CustomerApp.getInstance().getCurrentUser().getCustomer().getCustomerConfig().get(i).getConfigCode().equals("startType")) {
                    taskStartType = CustomerApp.getInstance().getCurrentUser().getCustomer().getCustomerConfig().get(i).getConfigValue();
                    taskStartDesc = CustomerApp.getInstance().getCurrentUser().getCustomer().getCustomerConfig().get(i).getConfigDesc();
                    break;
                }
            }
        }
        ...
        public void onStart(RoomListBean data, int position) {
            ...
            if (roomListBean.getTaskRoomState() == ConstantInt.ROOM_STATE_1) {
                if (taskStartType.equals(ConstantStr.START_TYPE_0)) {//无要求，直接开始任务
                    ...
                    CustomerApp.getInstance().showToast(taskStartDesc);
                }
            } else {
                startReportActivity();
            }
        ...
        switch (v.getId()) {
            case R.id.tv_finish_inspection:
                ...
                if (finishCount == mList.size()) {
                    ...
                    CustomerApp.getInstance().showToast("还有没有完成的巡检");
                }
                break;
        }
    }
