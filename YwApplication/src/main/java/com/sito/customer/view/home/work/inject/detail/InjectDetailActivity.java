...
import com.sito.customer.mode.inject.bean.EquipmentInjectionOilBean;
...
import com.sito.customer.view.home.work.inject.oil.InjectOilActivity;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        if (CustomerApp.getInstance().getCurrentUser().getCustomer().getCustomerConfig() != null
            ...
            for (int i = 0; i < CustomerApp.getInstance().getCurrentUser().getCustomer().getCustomerConfig().size(); i++) {
                if (CustomerApp.getInstance().getCurrentUser().getCustomer().getCustomerConfig().get(i).getConfigCode().equals("defaultNeedInjectionDays")) {
                    String time = CustomerApp.getInstance().getCurrentUser().getCustomer().getCustomerConfig().get(i).getConfigValue();
                    try {
                        defaultNeedInjectionDays = Integer.valueOf(time);
                    } catch (Exception e) {
                        e.printStackTrace();
                    }
                    break;
        ...
        loadLayout = (RecycleRefreshLoadLayout) findViewById(R.id.refreshLoadLayoutId);
        ...
        expendRecycleView = (ExpendRecycleView) findViewById(R.id.recycleViewId);
        expendRecycleView.setLayoutManager(new GridLayoutManager(this, 1));
        TextView equipmentName = (TextView) findViewById(R.id.tvEquipmentName);
        TextView tvEquipmentSn = (TextView) findViewById(R.id.tvEquipmentSn);
        TextView tvInjectState = (TextView) findViewById(R.id.tvInjectState);
        TextView tvInjectTime = (TextView) findViewById(R.id.tvInjectTime);
        TextView tvBackInject = (TextView) findViewById(R.id.tvBackInject);
        TextView tvBeforeInject = (TextView) findViewById(R.id.tvBeforeInject);
        ...
        RVAdapter<InjectEquipmentLog.ItemList> adapter = new RVAdapter<InjectEquipmentLog.ItemList>(expendRecycleView
            ...
            public void showData(ViewHolder vHolder, InjectEquipmentLog.ItemList data, int position) {
                ...
                injectName.setText(DataUtil.timeFormat(data.getCreateTime(), "yyyy-MM-dd")
                ...
                for (int i = 0; i < data.getInjectionOilDataList().size(); i++) {
                    ...
                    textView.setPadding(0, DisplayUtil.dip2px(InjectDetailActivity.this, 10), 0, 0);
