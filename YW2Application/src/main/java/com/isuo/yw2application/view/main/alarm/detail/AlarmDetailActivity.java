...
import com.isuo.yw2application.widget.PlaySoundLayout;
import com.isuo.yw2application.widget.ProgressItemLayout;
import com.isuo.yw2application.widget.ProgressTitleLayout;
        ...
        if (isAwait) {
            ...
            if (alarmCardView.getVisibility() == View.VISIBLE) {
                ...
                if (Yw2Application.getInstance().getCurrentUser().getCustomer().getIsOpen() == 1) {
                    ...
                    titleTv[0] = tvLeft;
                    titleTv[1] = tvRight;
                } else if (Yw2Application.getInstance().getCurrentUser().getIsRepair() != 1) {
                    titleTv = new TextView[2];
                    tvCenter.setVisibility(View.GONE);
                    tvLeft.setText(R.string.title_alarm_detail_1);
                    tvRight.setText(R.string.title_alarm_detail_2);
        ...
        switch (v.getId()) {
            ...
            case R.id.tv_upload:
                if (careType == 1 && TextUtils.isEmpty(careEndTime)) {
                    Yw2Application.getInstance().showToast("请选择关注截止日期");
                    return;
                }
                if (actionType == 1) {
                    ...
                    if (Yw2Application.getInstance().getCurrentUser().getCustomer().getIsOpen() == 1) {
                        ...
                        if (mFaultDetail.getDefaultFlowId() == null) {
                            Yw2Application.getInstance().showToast("未配置流转人员");
                            return;
                        } else {
                        ...
                        if (TextUtils.isEmpty(getUserIds())) {
                            Yw2Application.getInstance().showToast("请选择人员");
                            return;
                        } else {
                    ...
                    try {
                        ...
                        if (TextUtils.isEmpty(getUserIds())) {
                            Yw2Application.getInstance().showToast("请选择检修人员");
                            return;
                        }
                        jsonObject.put("usersNext", getUserIds());
                        if (TextUtils.isEmpty(startTime)) {
                            Yw2Application.getInstance().showToast("请选择开始时间");
                            return;
                        }
                        jsonObject.put("startTime", startTime);
                        if (TextUtils.isEmpty(endTime)) {
                            Yw2Application.getInstance().showToast("请选择结束时间");
                            return;
                        }
        ...
        ll_choose_pack.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                if (jobPackageBeen == null || jobPackageBeen.getList() == null || jobPackageBeen.getList().size() == 0) {
                    Yw2Application.getInstance().showToast("没有作业包,无法选择");
                    return;
                }
