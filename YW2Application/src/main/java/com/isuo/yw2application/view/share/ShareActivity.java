package com.isuo.yw2application.view.share;
...
public class ShareActivity extends BaseActivity {
    ...
    private String mUrl = "/customer/invitation.json?customerId="
            + Yw2Application.getInstance().getCurrentUser().getCustomer().getCustomerId();
    ...
    public boolean onOptionsItemSelected(MenuItem item) {
        if (item.getItemId() == R.id.menu_share) {
            if (!isAppInstalled(this.getApplicationContext(), "com.tencent.mm")) {
                Yw2Application.getInstance().showToast("没有安装微信");
                return true;
            }
            ...
            Yw2Application.iwxapi.sendReq(req);
            return true;
        }
