...
public class IncrementRepository implements IncrementDataSource {
    ...
    public Subscription loadIncrementFromDb(final long workId, @NonNull final LoadIncrementDataCallBack callBack) {
        return Yw2Application.getInstance().getDaoSession()
                .getImageDao().queryBuilder()
                .where(ImageDao.Properties.CurrentUserId.eq(Yw2Application.getInstance().getCurrentUser().getUserId())
                ...
                .flatMap(new Func1<List<Image>, Observable<Voice>>() {
                    ...
                    public Observable<Voice> call(List<Image> imageList) {
                        return Yw2Application.getInstance().getDaoSession().getVoiceDao().queryBuilder()
                                .where(VoiceDao.Properties.CurrentUserId.eq(Yw2Application.getInstance().getCurrentUser().getUserId())
    ...
    public void saveVoice(Voice voice) {
        if (voice != null) {
            Yw2Application.getInstance().getDaoSession().getVoiceDao().rx().insertOrReplace(voice).subscribe();
        }
    }
    ...
    public void cleanVoiceData(Voice voice) {
        if (voice != null) {
            Yw2Application.getInstance().getDaoSession().getVoiceDao().deleteInTx(voice);
        }
    }
    ...
    public void cleanImageData(List<Image> imageList) {
        if (imageList != null) {
            Yw2Application.getInstance().getDaoSession().getImageDao().deleteInTx(imageList);
        }
    }
