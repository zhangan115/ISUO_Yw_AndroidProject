...
import com.sito.evpro.inspection.widget.MyAF;
...
public class FaultHistoryActivity extends BaseActivity implements FaultHistoryContract.View, SwipeRefreshLayout.OnRefreshListener, RecycleRefreshLoadLayout.OnLoadListener {
    //view
    ...
    @Inject
    FaultHistoryPresenter mFaultHistoryPresenter;
    FaultHistoryContract.Presenter mPresenter;

    @Override
    protected void onCreate(Bundle savedInstanceState) {
        super.onCreate(savedInstanceState);
        setLayoutAndToolbar(R.layout.activity_fault_history, "历史故障");
        DaggerFaultHistoryComponent.builder().inspectionRepositoryComponent(InspectionApp.getInstance().getRepositoryComponent())
                .faultHistoryModule(new FaultHistoryModule(this)).build()
                .inject(this);
        initView();
        initData();
    }
    ...
    private void initData() {
        ...
        RVAdapter<FaultList> adapter = new RVAdapter<FaultList>(mExpendRecycleView, mList, R.layout.item_fault_history) {
            ...
            public void showData(ViewHolder vHolder, final FaultList data, final int position) {
                ...
                mTvSeconds.setText(data.getSoundTimescale() + "''");
                ...
                mFaultType.setText(InspectionApp.getInstance().getMapOption().get(ConstantInt.FAULT + "").get(data.getFaultType() + ""));
                ...
                if (data.isPlay() && isPlay) {
                    ...
                    mCountDownTimerUtils = new CountDownTimerUtils(mTvSeconds, data.getSoundTimescale() * 1000, 1000, data.getSoundTimescale() + "''", "#ffffff");
    ...
    private ImageView getImageView(int position, String[] url) {
        int width = InspectionApp.getInstance().getDisplayWidth();
