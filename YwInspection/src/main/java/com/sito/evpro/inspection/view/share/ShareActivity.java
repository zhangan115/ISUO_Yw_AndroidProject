package com.sito.evpro.inspection.view.share;
...
public class ShareActivity extends BaseActivity {
    ...
    private String mUrl = "http://114.118.94.117:8080/isuoyw/api/customer/invitation.json?customerId="
            + InspectionApp.getInstance().getCurrentUser().getCustomer().getCustomerId();
                    ...
                    if (!Thread.currentThread().isInterrupted()) {
                        switch (msg.what) {
                            case 0:
                                ...
                                break;
                            case 1:
                                mProgressD.dismiss();//隐藏进度对话框，不可使用dismiss()、cancel(),否则再次调用show()时，显示的对话框小圆圈不会动。
                                break;
                        }
                    }
