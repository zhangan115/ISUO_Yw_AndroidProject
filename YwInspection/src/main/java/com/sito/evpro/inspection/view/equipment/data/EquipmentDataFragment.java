...
import com.sito.evpro.inspection.mode.bean.equip.ChartData;
...
import com.sito.evpro.inspection.widget.ChartDialog;
import com.sito.evpro.inspection.widget.CheckDialog;
...
public class EquipmentDataFragment extends MvpFragment<EquipmentDataContact.Presenter> implements EquipmentDataContact.View, IViewPagerChangeListener, SwipeRefreshLayout.OnRefreshListener {
    ...
    public void showCheckData(InspectionData inspectionData) {
        ...
        if (inspectionData.getPicList() != null && inspectionData.getPicList().size() > 0) {
            ...
            checkData.setValue(inspectionData.getPicList().get(0).getInplacePicUrl());
            checkData.setInspectionType(5);
            String[] photoUrls = new String[inspectionData.getPicList().size()];
            for (int i = 0; i < inspectionData.getPicList().size(); i++) {
                photoUrls[i] = inspectionData.getPicList().get(i).getInplacePicUrl();
            }
            checkData.setPhotos(photoUrls);
