...
public class EquipmentRecordFragment extends MvpFragment<EquipmentRecordContact.Presenter> implements EquipmentRecordContact.View, IViewPagerChangeListener, SwipeRefreshLayout.OnRefreshListener, RecycleRefreshLoadLayout.OnLoadListener {
    ...
    public void onViewCreated(View view, @Nullable Bundle savedInstanceState) {
        ...
        RVAdapter<OverhaulBean> adapter = new RVAdapter<OverhaulBean>(mExpendRecycleView, mList, R.layout.item_overhaul) {
            ...
            public void showData(ViewHolder vHolder, OverhaulBean data, int position) {
                ...
                TextView tv_start_work = (TextView) vHolder.getView(R.id.tv_start_work);
                tv_start_work.setVisibility(View.GONE);
                ...
                if (!TextUtils.isEmpty(InspectionApp.getInstance().getMapOption().get("4").get(String.valueOf(data.getRepairResult())))) {
                    tv_repair_result.setVisibility(View.VISIBLE);
                    startLayout.setVisibility(View.VISIBLE);
                    tv_repair_result.setText(MessageFormat.format("检修结果:{0}"
                            , InspectionApp.getInstance().getMapOption().get("4").get(String.valueOf(data.getRepairResult()))));
                } else {
